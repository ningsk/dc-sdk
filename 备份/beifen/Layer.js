...
var getOneKey = function(arr) {
  var n = Math.floor(Math.random() * arr.length + 1) - 1;
  return arr[n];
}
class Layer {
  static regLayerForConfig(type, layerClass) {
    exLayer[type] = layerClass;
  }
  //创建图层
  static createLayer(viewer, item, serverURL) {
    var layer;
  ...
  //创建地图底图
  static createImageryProvider = function(item, serverURL) {
    if (item.url) {
      if (serverURL) {
        item.url = item.url.replace('$serverURL$', serverURL);
      }
    ...
    switch (opts.type_new || opts.type) {
      ...
      case "mapbox": //mapbox
      case "www_mapbox":
        opts.accessToken = Cesium.defaultValue(opts.accessToken,
