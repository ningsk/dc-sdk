...
class MapSplit extends Widget {
   ...
   */
  _installHook () {
    Object.defineProperty(this._map, 'mapSplit', {
      value: this,
      writable: false
    })
  }
   ...
   */
  _bindEvent () {
    this._map.scene.imagerySplitPosition = 0.5
  ...
  _unbindEvent () {
    if (this._baseLayer) {
      this._map.scene.imagerySplitPosition =
        this._baseLayer.splitDirection > 0 ? 1 : 0
    } else {
      this._map.scene.imagerySplitPosition = 0
    }
  }
   ...
   */
  _mountContent () {
    const splitter = DomUtil.parseDom(Icons.splitter, true, 'splitter')
    this._wrapper.appendChild(splitter)
    const handler = new Cesium.ScreenSpaceEventHandler(splitter)
    const self = this
  ...
  _moveHandler (movement) {
    ...
    this._map.scene.imagerySplitPosition = splitPosition
  }
  ...
  addBaseLayer (baseLayer, splitDirection = 1) {
    ...
    if (baseLayer) {
      this._baseLayer && this._map.imageryLayers.remove(this._baseLayer)
      this._baseLayer = this._map.imageryLayers.addImageryProvider(baseLayer)
