...
export var Tiles3dLayer = BaseLayer.extend({
  ...
  initData: function () {
    ...
    this.model = this.viewer.scene.primitives.add(
      new Cesium.Cesium3DTileset(this.config)
    );
    ...
    this.model.readyPromise.then((tileset) => {
      ...
      if (tileset._root && tileset._root.transform) {
        that.originMatrixInverse = Cesium.Matrix4.inverse(
  ...
  updateAxis: function (axis) {
    ...
    this.model._root.transform = Cesium.Matrix4.multiplyTransformation(
  ...
  updateMatrix: function (offsetopt) {
    ...
    if (
      ...
      if (isOK) {
        ...
        var cartographic = Cesium.Cartesian3.fromRadians(
  ...
  autoHeight: function (position, offsetopt) {
    ...
    Util.terrainPolyline({
      ...
      callback: (raisedPositions, noHeight) => {
        ...
        var point = PointUtil.formatPosition(raisedPositions[0]);
