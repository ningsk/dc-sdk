...
import { Util, DomUtil } from '@/dc/util'
import { MouseEventType } from '@/dc/event'
import { isBetween } from '@/dc'
class DivIcon extends Overlay {
    constructor (position, content) {
        ...
        this.type = OverlayType.DIV_ICON
    ...
    _updateStyle (style, distance) {
        ...
        if (style.transform) {
            const x = style.transform.x - this._delegate.offsetWidth / 2
            const y = style.transform.y - this._delegate.offsetHeight / 2
        ...
        if (distance && scaleByDistance) {
            const nearValue = scaleByDistance.nearValue
            const farValue = scaleByDistance.farValue
            const f = distance / scaleByDistance.far
    ...
    static fromEntity (entity, content) {
        ...
        const divIcon = new DivIcon(position, content)
        if (entity.billboard) {
            divIcon.attr = {
