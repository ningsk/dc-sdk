 ...
 * @LastEditors: 宁四凯
 * @LastEditTime: 2020-08-25 10:01:51
 */
...
class Layer {
	//创建图层
	static createLayer(item, viewer, serverURL, layerToMap) {
		var layer;
	...
	//创建地图底图
	static createImageryProvider(item, serverURL) {
		if (item.url) {
			if (serverURL) {
				item.url = item.url.replace('$serverURL$', serverURL);
			}
		...
		for (var key in item) {
			...
			switch (key) {
				default:
					...
					break;
				case "crs":
					if (value == "4326" || value.toUpperCase() == "EPSG4326") opts.tilingScheme = new Cesium.GeographicTilingScheme({
		...
		switch (opts.type_new || opts.type) {
			...
			case "www_tdt":
				...
				if (item.crs == '4326') {
					...
					var _url = 'http://t{s}.tianditu.gov.cn/' + _layer +
						'_c/wmts?service=WMTS&version=1.0.0&request=GetTile&tilematrix={TileMatrix}&layer=' + _layer +
						'&style={style}&tilerow={TileRow}&tilecol={TileCol}&tilematrixset={TileMatrixSet}&format=tiles&tk=' + _key;

					layer = new Cesium.WebMapTileServiceImageryProvider({
						url: opts.proxy ? new Cesium.Resource({
							url: _url,
							proxy: opts.proxy
						}) : _url,
						layer: _layer,
			...
			case "www_gaode":
				...
				switch (opts.layer) {
					...
					default:
						...
						_url = 'http://' + (opts.bigfont ? 'wprd' : 'webrd') +
