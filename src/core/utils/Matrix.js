...
class Matrix {
  ...
  static getHeadingPitchRollByOrientation(position, orientation) {
  ...
  static getHeadingPitchRollForLine(localStart, localEnd, ellipsoid) {
  ...
  static getRotateCenterPoint(center, point1, angle) {
    ...
    let Q = Cesium.Quaternion.fromAxisAngle(
      normaB,
    ...
    let A1 = Cesium.Cartesian3.subtract(
      point1,
      center,
      new Cesium.Cartesian3()
    );
  ...
  //获取点的offest平移矩阵后点
  static getPositionTranslation(position, offest, degree, type) {
    degree = degree || 0;
    type = type || "z";
