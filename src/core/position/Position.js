...
import { extend } from 'leaflet/src/core/Util';
 ...
 class Position {
    ...
    * @param {*} position 
    */
   static updateHeightForClampToGround(viewer, position) {
     //TODO viewer?
    var carto = Cesium.Cartographic.fromCartesian(position);
   ...
   static getExtent(viewer, isToWgs) {
    ...
    if (car3_rb) {
      ...
      for (var yIdx = canvas.height; yIdx >= yMax; yIdx -= 10) {
        var xIdx = yIdx >= xMax ? yIdx : xMax;
        car3_rb2 = view.camera.pickEllipsoid(new Cesium.Cartesian2(xIdx, yIdx), ellipsoid);
        if (car3_rb) break;
      }
      if (car3_rb2) {
   ...
   static getCameraView(viewer, isToWgs) {
    ...
    bookmark.heading = this.formatNum(Cesium.Math.toDegrees(camera.heading || -90), 1);
