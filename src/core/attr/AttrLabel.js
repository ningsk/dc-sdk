const { emptyImageUrl } = require("leaflet/src/core/Util");
const { Util } = require("../utils");
const { CornerType } = require("cesium");
    ...
    if (entityAttr == null) {
      ...
      for (var key in style) {
        ...
        switch (key) {
          ...
          case "color":
            //颜色
            entityAttr.fillColor = new Cesium.Color.fromCssColorString(value || "#ffffff").withAlpha(Number(
              style.opacity || 1.0));
            break;
          ...
          case "border_color":
            //衬色
            entityAttr.outlineColor = new Cesium.Color.fromCssColorString(value || "#000000").withAlpha(Number(
              style.opacity || 1.0));
            break;
          case "border_width":
          ...
          case "background_color":
            //背景色
            entityAttr.backgroundColor = new Cesium.Color.fromCssColorString(value || "#000000").withAlpha(
          ...
          case "distanceDisplayCondition":
            //是否按视距显示
            if (value) {
              entityAttr.distanceDisplayCondition = new Cesium.DistanceDisplayCondition(Number(style.distanceDisplayCondition_near ||
      ...
      //样式（倾斜、加粗等）
      var fontStyle = (style.font_style || "normal") + " small-caps " + (style.font_weight || "normal") + " " + (style
        .font_size || "25") + "px " + (style.font_family || "楷体");
