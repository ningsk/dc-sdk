  ...
  fse.exists(path.resolve(__dirname, '..', `packages/${item}/dist`), exists => {
    if (exists) {
      ...
      try {
        ...
        if (code !== 0) {
          shell.echo(chalk.red(` release @ningsk/dc-${item} v${version} failed`))
        } else {
          shell.echo(
            chalk.yellow(` release @ningsk/dc-${item} v${version} success`)
          )
        }
      } catch (e) {
        shell.echo(chalk.red(` release @ningsk/dc-${item} v${version} failed`))
      ...
      shell.echo(
        chalk.red(
          `no ${item} dist, release @ningsk/dc-${item} v${version} failed`
        )
      )
      if (index === count - 1) {
        shell.echo(chalk.green('release packages end'))
